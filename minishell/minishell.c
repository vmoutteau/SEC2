...
int executer(struct cmdline *cmd) {
    ...
    if (pidFils == 0) { // fils
        ...
        if (!cmd->backgrounded) {
            // Dans le cas où une tache n'est pas de fond, on attend le signal SIGCHLD
            pause();
        }
        else {
            // Dans le cas d'une tâche de fond, on passe à la suite directement
            printf("lancement d'une tache de fond\n");
        }
    }
    return EXIT_SUCCESS;
}
...
int main() {
    ...
    do {
        ...
        // Lire la commande.
        cmd = readcmd();
        printf("commande lancée : %s \n", getTexteCommande(cmd));
